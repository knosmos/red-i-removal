...
if __name__ == "__main__":
    ...
    cv2.imshow("input", cv2.cvtColor((sample['input']*255).permute(1, 2, 0).numpy().astype(np.uint8), cv2.COLOR_RGB2BGR))
    cv2.imshow("gt", cv2.cvtColor((sample['gt']*255).permute(1, 2, 0).numpy().astype(np.uint8), cv2.COLOR_RGB2BGR))
    cv2.waitKey(0)