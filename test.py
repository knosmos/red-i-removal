...
def visualize_samples(inputs, targets, outputs):
    assert len(inputs) == len(targets) == len(outputs), "Inputs, targets, and outputs must have the same length."
    ...
    # plt.subplot(1, 3, 3)
    # plt.title("Output")
    for idx, (sample_input, sample_gt, sample_output) in enumerate(zip(inputs, targets, outputs)):
        plt.subplot(num_samples, 3, idx * 3 + 1)
        plt.imshow((sample_input.squeeze().permute(1, 2, 0)*255).cpu().numpy().astype(np.uint8))
        if idx == 0:
            plt.title("Input")
        plt.subplot(num_samples, 3, idx * 3 + 2)
        plt.imshow((sample_gt.squeeze().permute(1, 2, 0)*255).numpy().astype(np.uint8))
        if idx == 0:
            plt.title("Ground Truth")
        plt.subplot(num_samples, 3, idx * 3 + 3)
        plt.imshow((sample_output.squeeze().detach().permute(1, 2, 0)*255).cpu().numpy().astype(np.uint8))
        if idx == 0:
            plt.title("Output")
    plt.tight_layout()
